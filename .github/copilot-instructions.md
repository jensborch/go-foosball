...
- **`persistence/`**: Repository pattern with GORM
- **`resources/`**: HTTP handlers following RESTful patterns, WebSocket event publishers
- **Database**: SQLite with GORM AutoMigrate, single connection (SQLite limitation)
...
- **Connection limit**: `SetMaxOpenConns(1)` due to SQLite's single-writer limitation
...
3. **Round-robin logic** in `service/game_round_generator.go` manages fair game scheduling
